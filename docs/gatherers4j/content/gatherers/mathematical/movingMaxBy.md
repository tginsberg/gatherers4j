...
This implementation is suitable for comparing an arbitrary `Stream<T>` with a `Comparator`; for a version that operates directly on a `Stream<Comparable<T>>`, see [`movingMax()`](/gatherers4j/gatherers/mathematical/movingmax/).
...
Stream
    .of("3", "2", "1", "3", "4")
    .gather(Gatherers4j.<String>movingMaxBy(3, comparing(Integer::valueOf)).withOriginal())
    .toList();
