---
title: "movingStandardDeviationBy()"
linkTitle: "movingStandardDeviationBy()"
show_in_table: true
category: "Mathematical Operations"
description: Calculate the moving standard deviation of `BigDecimal` objects mapped from a `Stream<INPUT>` via a `mappingFunction` and looking back `windowSize` elements, as either a population or a sample.

---

### Implementation Notes
This implementation is suitable for mapping an arbitrary `Stream<INPUT>` to `BigDecimal` via a `mappingFunction`; for a version that operates directly on a `Stream<BigDecimal>`, see [`movingStandardDeviation()`](/gatherers4j/gatherers/mathematical/movingstandarddeviation/).
...
`movingStandardDeviationBy(Dataset dataset, int windowSize, Function<INPUT, BigDecimal> mappingFunction)`
* `dataset` - Either `Population` or `Sample`
* `windowSize` - How many trailing elements to calculate the standard deviation over at any given point in the stream, must be greater than 1
...
#### Moving standard deviation (population) of window size 3, mapped from an object
...
Stream
    ...
    .gather(Gatherers4j.movingStandardDeviationBy(Dataset.Population, 3, NamedValue::value))
    .toList();
...
//   BigDecimal("7.363574011458174")
//   BigDecimal("8.164965809277261")
// ]
```

#### Moving standard deviation (sample) of window size 3, mapped from an object
...
Stream
    ...
    .gather(Gatherers4j.movingStandardDeviationBy(Dataset.Sample, 3, NamedValue::value))
    .toList();
...
Stream
    ...
    .gather(Gatherers4j.movingStandardDeviationBy(Dataset.Population, 3, NamedValue::value).excludePartialValues())
    .toList();
...
Stream
    ...
    .gather(Gatherers4j.movingStandardDeviationBy(Dataset.Population, 3, NamedValue::value).treatNullAsZero())
    .toList();
...
Stream
    ...
    .gather(Gatherers4j.movingStandardDeviationBy(Dataset.Population, 3, NamedValue::value).treatNullAs(BigDecimal.TWO))
    .toList();
...
Stream
    ...
    .gather(Gatherers4j
        .movingStandardDeviationBy(Dataset.Population, 3, NamedValue::value)
...
Stream
    ...
    .gather(Gatherers4j.movingStandardDeviationBy(Dataset.Population, 3, NamedValue::value).withOriginal())
    .toList();
...
//   WithOriginal[original=NamedValue[name=fourth, value=20.0], calculated=7.363574011458174]
//   WithOriginal[original=NamedValue[name=fifth, value=30.0], calculated=8.164965809277261]
// ]
```
